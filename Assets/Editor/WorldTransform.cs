...
public class WorldTransform : UIBaseAlignmentInspector
{
    ...
    void OnEnable()
    {
        ...
        if (EditorPrefs.HasKey("CustomWorldTransformUnfold"))
            unfold = EditorPrefs.GetString("CustomWorldTransformUnfold") == "True";
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (p.vector3Value != t.position || s.vector3Value != t.transform.lossyScale)
        {
            ...
            string originLabel = "World Transform";
            unfold = EditorGUILayout.Foldout(unfold, originLabel);
            EditorPrefs.SetString("CustomWorldTransformUnfold", unfold.ToString());
            if (unfold)
            {
