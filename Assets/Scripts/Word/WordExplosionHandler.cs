...
public class ExplosionHandler : MonoBehaviour
{
    ...
    public void HandleExplosion()
    {
        ...
        {
            Debug.Log("[ExplosionHandler] affected objects # = " + col.name);

            if (col.CompareTag("Word"))
            {
                // Debug.Log("[ExplosionHandler] Destroying word: " + col.name);
                gameManager.AddScore(2);
                var word = col.GetComponent<FallingWord>();
                if (word) word.OnCrash();
            }
            else if (col.CompareTag("ExplodedLetter"))
            {
                // Debug.Log("[ExplosionHandler] Destroying letter: " + col.name);
